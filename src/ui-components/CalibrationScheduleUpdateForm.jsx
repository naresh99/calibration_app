...
import { getCalibrationSchedule } from "../graphql/queries";
import { updateCalibrationSchedule } from "../graphql/mutations";
export default function CalibrationScheduleUpdateForm(props) {
  const {
    id: idProp,
    calibrationSchedule: calibrationScheduleModelProp,
    onSuccess,
    onError,
    onSubmit,
    onValidate,
  ...
  const resetStateValues = () => {
    const cleanValues = calibrationScheduleRecord
      ? { ...initialValues, ...calibrationScheduleRecord }
      : initialValues;
    setSchedule_name(cleanValues.schedule_name);
    setSchedule_number(cleanValues.schedule_number);
    setInstrument_type(cleanValues.instrument_type);
    setCalibration_frequency(cleanValues.calibration_frequency);
    setEffective_Date(cleanValues.effective_Date);
    setYear(cleanValues.year);
    setReference_sop(cleanValues.reference_sop);
    setPrepared_by(cleanValues.prepared_by);
    setPrepared_date(cleanValues.prepared_date);
    setChecked_by(cleanValues.checked_by);
    setChecked_date(cleanValues.checked_date);
    setApproved_by(cleanValues.approved_by);
    setApproved_date(cleanValues.approved_date);
    setErrors({});
  };
  const [calibrationScheduleRecord, setCalibrationScheduleRecord] =
    React.useState(calibrationScheduleModelProp);
  React.useEffect(() => {
    const queryData = async () => {
      const record = idProp
        ...
        : calibrationScheduleModelProp;
      setCalibrationScheduleRecord(record);
    };
    queryData();
  }, [idProp, calibrationScheduleModelProp]);
  React.useEffect(resetStateValues, [calibrationScheduleRecord]);
  const validations = {
  ...
  return (
    <Grid
      ...
      onSubmit={async (event) => {
        ...
        let modelFields = {
          schedule_name,
          schedule_number: schedule_number ?? null,
          instrument_type: instrument_type ?? null,
          calibration_frequency: calibration_frequency ?? null,
          effective_Date: effective_Date ?? null,
          year: year ?? null,
          reference_sop: reference_sop ?? null,
      ...
      {...getOverrideProps(overrides, "CalibrationScheduleUpdateForm")}
      {...rest}
    >
      <TextField
        label="Schedule name"
      ...
      >
        <Button
          ...
          isDisabled={!(idProp || calibrationScheduleModelProp)}
